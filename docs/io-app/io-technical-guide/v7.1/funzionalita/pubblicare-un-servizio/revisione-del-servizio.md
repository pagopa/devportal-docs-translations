...
2. At the top, you will find an information box indicating the [service status](stato-del-servizio.md).
...
2. Next to the service name, you will find a tag indicating the [service status](stato-del-servizio.md);
...
2. Query the API to [request a review](../../api-e-specifiche/api-servizi/manage-service-request-review.md). You can choose to proceed with automatic publication of the service in case of approval.
...
1. Query the API to [read the details of a service](../../api-e-specifiche/api-servizi/manage-service-get.md);
2. Consult the `status.value` field to know the outcome of the review;
3. Understand what the possible [service statuses](stato-del-servizio.md) are.
