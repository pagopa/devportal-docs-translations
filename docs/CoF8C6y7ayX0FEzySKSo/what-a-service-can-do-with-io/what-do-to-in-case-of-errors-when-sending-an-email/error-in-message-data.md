...
* **send a test message to multiple colleagues**, following the[ instructions in the technical guide](https://app.gitbook.com/s/I8hgrCWjc4Pg8hUMK0Pq/function).
...
If the message includes a payment notice, before sending it first check that the relative payments are due. Otherwise, the citizens will receive a notice that they cannot pay.
{% endhint %}
...
{% hint style="info" %}
\*\*Any questions?\*\* Before sending an error correction message, \*\*contact IO support\*\*, accessing your \[\*\*reserved area for institutions\*\*]\(https://selfcare.pagopa.it/auth/login) and pressing \*\*support\*\*.
