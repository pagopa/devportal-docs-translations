...
To send a message , you must first [publish-a-service.md](publish-a-service.md "mention").
...
\{% hint style="info" %\} **Example:** all the APIs can return `status code 429`, which represents a signal that indicates the exceedance of the permitted rate. In this case, it is necessary to implement a _retry_ mechanism and decrease the _rate_ of the requests by inserting pauses.
