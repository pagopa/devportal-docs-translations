...
{% hint style="warning" %}
Different opportunities must be associated with different static code lists for greater security. The static codes must be accepted by the operator on its systems until they are used for the entire duration of the opportunity offered to the beneficiaries.
{% endhint %}
...
{% hint style="info" %}
The "new" code list will be displayed only after all the previously uploaded codes have been used.
{% endhint %}
...
{% hint style="warning" %}
When the discount code is displayed in the IO app, it is considered used regardless if it is actually entered or used on the operator's e-commerce website, therefore it will no longer be displayed to beneficiaries (including the same beneficiary if they view the code for that opportunity at a later moment).
{% endhint %}
