...
{% hint style="info" %}
All the APIs described on this page refer to the Entity's activities via the [Private Area](https://app.gitbook.com/o/KXYtsf32WSKm6ga638R3/s/axttcUGV65V2IVRggmvR/) and are based on obtaining and using the ["manage" key](../../Features/Publishing-a-Service/Manage-Key/Manage-Key.md) which you can [retrieve](../../Features/Publishing-a-Service/Manage-Key/Manage-Key.md#recupera-la-chiave-manage) in the IO Services management area.
{% endhint %}
...
{% hint style="success" %}
Refer to the [#examples](CRU-Remote-Configurations.md#esempi "mention") section for more information on field usage and documented use cases.
{% endhint %}

### "Create" API

With this API, you can create a new configuration to obtain the `configuration_id` to be used in all subsequent calls, including for [creating messages with remote content](../../Features/Sending-a-Message/Sending-a-Message-with-Remote-Content.md).
...
The **`has_precondition`** field governs IO's interpretation of the corresponding [#has_precondition](Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#has_precondition "mention") field that you will specify when creating a remote content message, according to the following schema:
...
In response, you will get the **configuration identifier** in the `"configuration_id"` field, which you must use in all subsequent calls, including those for [creating remote content messages](../../Features/Sending-a-Message/Sending-a-Message-with-Remote-Content.md) that are to be served by it.
