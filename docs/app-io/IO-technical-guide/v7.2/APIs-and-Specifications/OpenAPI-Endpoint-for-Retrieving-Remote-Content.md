...
{% hint style="info" %}
Read [this page](../Features/Sending-a-Message/Sending-a-Message-with-Remote-Content.md) to learn more about remote content messages.
{% endhint %}
...
If, during [#creazione-del-messaggio-con-contenuto-remoto](../Features/Sending-a-Message/Sending-a-Message-with-Remote-Content.md#creazione-del-messaggio-con-contenuto-remoto "mention"), you set the [#has\_precondition](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#has_precondition "mention") field (or if you indicated it during [configurazione-remota.md](../setup-iniziale/configurazione-remota.md "mention")), when the recipient opens the message, IO will retrieve the preconditions through a `GET` call to your systems, containing the following input parameters:

- the remote correlation `id` that you specified in the [#third\_party\_data](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#third_party_data "mention") block when sending the message;
- the recipient's fiscal code (as a header).

The subscriber is responsible for verifying the input fiscal code, through an appropriate call ([Get a User Profile using POST](Message-APIs/Get-a-User-Profile-Using-POST.md)); if the outcome is positive, the preconditions content will be sent to IO.

<figure><img src="../.gitbook/assets/Sequenza recupero precondizioni remote.png" alt=""><figcaption></figcaption></figure>

IO will use the `base_url`, which you provided when setting up the remote configuration information, and the correlation identifier, which you specified in the [#third\_party\_data](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#third_party_data "mention") block when sending the message, to compose a GET call in the form `{base_url}/messages/{id}/precondition:`
...
If, during [#creazione-del-messaggio-con-contenuto-remoto](../Features/Sending-a-Message/Sending-a-Message-with-Remote-Content.md#creazione-del-messaggio-con-contenuto-remoto "mention"), you included the [#has\_remote\_content](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#has_remote_content "mention") flag or, as a Premium Service Provider, the [#has\_attachments](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#has_attachments "mention") flag, IO will need to retrieve the message content from your systems when it is viewed in the app, and it will use the `GET` API with the following parameters:
...
IO will use the `base_url` created when setting up the remote configuration information, and the correlation identifier, specified in the [#third\_party\_data](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#third_party_data "mention") block when sending the message, to compose a GET call in the form `{base_url}/messages/{id}:`
...
Depending on the flags you specified in [#third\_party\_data](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#third_party_data "mention") at the time of [#creazione-del-messaggio-con-contenuto-remoto](../Features/Sending-a-Message/Sending-a-Message-with-Remote-Content.md#creazione-del-messaggio-con-contenuto-remoto "mention"), your API response must include:
...
<figure><img src="../.gitbook/assets/image (19).png" alt="" width="375"><figcaption><p>How the example will appear in the app</p></figcaption></figure>
...
{% hint style="info" %}
When composing and sending the message text in markdown format, remember to set the `UTF-8` charset (to ensure the correct display of accented characters) and refer to the [Markdown Guide](../Useful-Resources/Markdown-Guide.md)
{% endhint %}
...
- `{id}` is the identifier you specified in the [#third\_party\_data](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#third_party_data "mention") block when sending the message;
...
{% hint style="warning" %}
Please note: in this case, the unique identifier is the **remote correlation** `id` that you specified in [#third\_party\_data](Message-APIs/Submit-a-Message-Passing-the-User-Fiscal-Code-in-the-Request-Body.md#third_party_data "mention") when sending the message.
{% endhint %}
